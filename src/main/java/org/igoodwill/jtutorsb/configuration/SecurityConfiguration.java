...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/webjars/**", "/js/**").permitAll().antMatchers("/user/register")
				...
				.access("hasRole('ROLE_ADMIN') or hasRole('ROLE_TUTOR')").anyRequest().authenticated().and()
				.antMatcher("/?lang=ua").formLogin().loginPage("/login").failureUrl("/login?error").permitAll().and()
